...
def main():
    parser = argparse.ArgumentParser(
        description="Energy Demand Forecasting with Granite TinyTimeMixer"
    )
    ...
    if args.data_path and args.data_path.exists():
        ...
        df = prepare_energy_data(
            df, config["data"]["date_column"], config["data"]["demand_column"]
        )
        ...
        base_demand = (
            1000
            + 200 * np.sin(np.arange(config["data"]["n_periods"]) / (24 * 7))
            + 100 * np.sin(np.arange(config["data"]["n_periods"]) / 24)
        )
        demand = pd.Series(
            base_demand + np.random.normal(0, 50, config["data"]["n_periods"]),
            index=dates,
        )
    else:
